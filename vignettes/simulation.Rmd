...
First, users can make a complete object non-interactable by setting the `interactable` argument of the constructors for `rectangle`s, `polygon`s, and `circle`s to `FALSE`, such as in the following examples:
...
Once an environment has been defined, one should define the characteristics of the agents that are expected to walk around in this environment. The mapping of the setting with the agents is achieved through specifying an instance of the [`predped`](https://ndpvh.github.io/predped/reference/predped.html) class. It has the following attributes:
...
Note that this matrix is <u>not</u> a covariance matrix. Instead, it contains the standard deviations of each parameter on its diagonal and the correlations between each parameter on its off-diagonal, thus containing the following structure:
