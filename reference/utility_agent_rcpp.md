...
This function is the Rcpp equivalent of `utility-agent`. This function
uses the operational-level utility functions to compute the utility of
moving to any given potential cell in `centers`. Here, we assume that
none of the utility variables (i.e., the variables that serve as input
to the utility functions) is precomputed, so that it will first compute
their values. This input is then provided to `utility-data.frame` for
the actual computation of the utility.
