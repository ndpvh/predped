...
This variable contains the result of running `create_edges` in a
particular environment while setting the option `many_nodes = FALSE` and
without evaluating which edges cannot serve as navigation lines due to
objects being in the way. It mainly serves as a variable used to
benchmark some of the functions related to routing.
