...
utility <- function(agent, 
    ...
    if(!precomputed) {
        # Preferred speed
        goal_distance <- dist1(agent@position, 
                               agent@goals[attr(agent@goals, "i"), 1:2, drop = FALSE])
                               #state$P[[n]][attr(state$P[[n]], "i"), 1:2, drop = FALSE])

        # Angle between agent and the goal
        direction_goal <- goal_direction(agent) / 90

        # Interpersonal distance between agent and other agents
        interpersonal_distance <- others_close(agent, state, agent_predictions, centers)

        # Predict which directions might lead to collisions in the future
        blocked_angle <- predict_blocking(agent, state, agent_predictions)

        # Follow the leader phenomenon
        leaders <- get_leaders(agent, state, centers)

        # Walking besides a buddy
        buddies <- get_buddy(agent, state, agent_predictions, centers)
    ...
    V <- utility_preferred_speed(agent, goal_distance) +
        utility_goal_direction(agent, direction_goal) + 
        utility_current_direction(agent) +
        utility_interpersonal_distance(agent, interpersonal_distance, check) + 
        utility_blocked_angle(agent, blocked_angle) +
        utility_follow_leader(agent, leaders) +
        utility_buddy(agent, buddies)
    
    # Stop baseline (set by gwUtility) and scaling
    V_transformed <- c(-agent@parameters["bS"], V) / agent@parameters["rU"]
