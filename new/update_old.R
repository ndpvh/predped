        ...
        for (g in unique_groups) {
            ...
            if (length(g_indices) > 1) {
                ...
                if (new_rep_idx != current_rep_idx) {
                    ...
                    # --- NEW: Clear the path so the new rep replans from their position ---
                    # The old rep's path was computed from their location; new rep needs their own
                    current_goal(new_rep_agent)@path <- matrix(nrow = 0, ncol = 2)
                    
                    # Force status to "plan" if not already close to goal
...
update_goal <- function(agent,
    ...
    if((distance_path_point <= close_enough) & !(status(agent) %in% c("exit", "completing goal"))) {
        ...
        if(current_goal(agent)@id == "goal exit") {
            status(agent) <- "exit"
        } else if (group_representative(agent) || (id(current_goal(agent)) != id(current_group_goal(agent)))) {
            # --- NEW: Rep interacts with group goals, ANYONE interacts with private goals ---
            status(agent) <- "completing goal"                    
            co_1 <- position(agent)
            co_2 <- current_goal(agent)@position 
            orientation(agent) <- atan2(co_2[2] - co_1[2], co_2[1] - co_1[1]) * 180 / pi
        } else {
            # --- NEW: Followers wait patiently nearby ONLY for shared group goals ---
            status(agent) <- "wait"
    ...
    if(status(agent) == "completing goal") {
        ...
        if(current_goal(agent)@done) {
            ...
            has_group_goals <- length(goals(agent)) > 0
            has_indiv_goals <- length(individual_goals(agent)) > 0
            
            if(has_group_goals || has_indiv_goals) {
                
                next_is_individual <- FALSE
                selected_idx <- 1
                
                # 1. Spatial check for surrounding individual goals
                if (has_indiv_goals) {
                    pos_agent <- position(agent)
                    
                    # Calculate Euclidean distances to all private goals
                    dist_indiv <- sapply(individual_goals(agent), function(g) {
                        m4ma::dist1(pos_agent, matrix(g@position, ncol = 2))
                    })
                    
                    min_dist_indiv <- min(dist_indiv)
                    closest_indiv_idx <- which.min(dist_indiv)
                    
                    # Define the opportunity radius (e.g., within 15 units)
                    search_radius <- 5.0 
                    
                    if (has_group_goals) {
                        if (min_dist_indiv < search_radius) {
                            # Only detour if the private goal is closer than the next group goal
                            dist_group <- m4ma::dist1(pos_agent, matrix(goals(agent)[[1]]@position, ncol = 2))
                            if (min_dist_indiv < dist_group) {
                ...
                # 2. Apply the chosen goal
                if (next_is_individual) {
                    # Divert to the individual goal
    ...
    if(status(agent) == "wait") {
        # --- NEW: Check if agent should switch to an individual goal ---
        # While waiting, followers can switch to nearby individual goals (within 5 units)
        if (!group_representative(agent) && length(individual_goals(agent)) > 0 && 
            !is.null(current_goal(agent)) && !is.null(current_group_goal(agent)) &&
            id(current_goal(agent)) == id(current_group_goal(agent))) {
            
            pos_agent <- position(agent)
            
            # Calculate distances to all individual goals
            dist_indiv <- sapply(individual_goals(agent), function(g) {
                m4ma::dist1(pos_agent, matrix(g@position, ncol = 2))
            })
            ...
            # If an individual goal is within 5 units, switch to it
            if (min_dist_indiv <= 5.0) {
                closest_indiv_idx <- which.min(dist_indiv)
        ...
        if(waiting_counter(agent) < 0 & status(agent) != "move" & adaptive_goal_sorting) {
            # --- NEW: Followers can break free by pursuing individual goals ---
            if (!group_representative(agent)) {
                ...
                if (length(individual_goals(agent)) > 0) {
                    ...
                    current_group_goal(agent) <- goal()  # Empty goal
                    status(agent) <- "plan"
                    return(agent)  # Return early to replan
                } else {
                    # No individual goals available, keep waiting
                    waiting_counter(agent) <- 5
                }
            } else {
                if(length(goals(agent)) != 0) {
