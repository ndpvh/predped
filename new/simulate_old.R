...
#' @param individual_goal_number Numeric denoting how many of the
#' \code{goal_number} goals should be individual (private) goals. These goals
#' will only be pursued by a single agent in a group. Must not exceed
#' \code{goal_number}. Defaults to \code{0}.
...
setMethod("simulate", "predped", function(object,
    ...
    # Validate that individual_goal_number is not larger than goal_number
    if(individual_goal_number > min(goal_number)) {
        stop(paste0("individual_goal_number (", individual_goal_number, ") cannot be larger than ",
                    "the minimum goal_number (", min(goal_number), ")."))
    }
...
add_group <- function(model,
    ...
    # Extract the position of the first agent
    rep_pos <- position(agents[[1]])

    # Loop over these agents
    for(i in 2:agent_number) {
        # Create a temporary agent

        tmp_agent <- add_agent(model,
                               position = rep_pos,
                               standing_start = standing_start,
                               individual_differences = individual_differences,
                               individual_goal_number = individual_goal_number,
        ...
        # Force these agents into the same group
        group(tmp_agent) <- group(agents[[1]])

        # Designate their goals to be that of the first agent
        current_goal(tmp_agent) <- current_goal(agents[[1]])
        goals(tmp_agent) <- goals(agents[[1]])

        # Designate as a follower
        group_representative(tmp_agent) <- FALSE
        current_group_goal(tmp_agent) <- current_goal(agents[[1]])
        group_goals(tmp_agent) <- goals(agents[[1]])
...
add_agent <- function(model,
    ...
    if(is.null(precomputed_goals)) {
        ...
        if(check_1 & check_2) {
            ...
            total_goal_number <- goal_number

            goal_stack <- goal_stack(total_goal_number,
                                     background,
        ...
        # Partition the Goal Stack
        n_goals <- length(goal_stack)
        if (individual_goal_number > 0 && n_goals > 1) {
            # Partition goals: first (n_goals - individual_goal_number) are shared,
            # remaining individual_goal_number are private
            n_shared <- n_goals - individual_goal_number
            
            shared_goals <- goal_stack[1:n_shared]
            private_goals <- goal_stack[(n_shared + 1):n_goals]
        } else {
            shared_goals <- goal_stack
            private_goals <- list()
        }
    } else {
        ...
        # Partition the precomputed Goal Stack
        n_goals <- length(goal_stack)
        if (individual_goal_number > 0 && n_goals > 1) {
            # Partition goals: first (n_goals - individual_goal_number) are shared,
            # remaining individual_goal_number are private
...
create_initial_condition <- function(agent_number,
    ...
    for(i in seq_len(agent_number)) {
        ...
        if(i %in% group_indices | i == 1) {
            ...
            individual_goals(dummy) <- new_agent$individual_goals
            ...
            individual_goals(dummy) <- individual_goals(agents[[i - 1]])
