...
agents <- c(BaselineEuropean = 1/4,
            BigRushingDutch = 1/4,
            DrunkAussie = 1/4,
            CautiousOldEuropen = 1/4 )
...
# Make a single pedestrian in the supermarket and plot the 
# current state
#
# Is kind of a proof of concept: Shows what we can do already
plotPed(2 + state$p,                                    # Position of the pedestrian
        getP(state),                                    # The current goal
        state$a,                                        # Angle in which they are moving
        state$r,                                        # Width of the pedestrian
        objects,                                        # Objects in the supermarket
        types = attr(state$pMat, "types"),              # Attributes of the agent
        amult = 3,                                      # Length of the lines that show the angles
        body_border = "black" )                       # Border of the body of the pedestrian
# `state$pMat` gives the parameter matrix for the agent

# Here, create the same pedestrian, but also show what 
# options he has for moving around
draw_grid(2 + state$p[1,],                              # Position of the pedestrian 
          state$v * 3,                                  # Velocity of the pedestrian
          state$a,                                      # Angle in which the pedestrian is moving
          plotPoints = TRUE )                           # Points in the middle of the options where they might move to

# Add the waypoints to this plot drawn by `draw_grid`
points(pathPoints,                                      # Positions of the way points themselves
       pch = 3, 
       lwd = 2, 
       col = "#252525", 
       cex = 1.4 )
...
#
# Similar idea as the waypoints, only this time with the goals
#
# state$P[[1]] contains a dataframe with the goals and their positions
# in them. These goals are marked by the letter "G", which is what is
# selected on here
#
# TO DO: Make a function that will select all goals automatically instead
#        of having to do this yourself
points(state$P[[1]][substr(row.names(state$P[[1]]), 1, 1) == "G", ], 
       pch = 16, 
       col = "#CA0020", 
       cex = 2 )

# Get the edges of the goals that we used: 
# stacks[[1]] changed with state$P, which is the actual thing we want
# I believe
edges <- attributes(state$P[[1]])$replan$edges

# Draw where all paths may go
drawGoalPaths(edges,delay=0)

# Shows how the pedestrian walked from one goal to another to eventually 
# the exit
set.seed(1)
tour10 <- getGoalStack(10,                                      # Number of objects to get
                       pSwap = 0,                               # Tendency to swap goals
                       pDetour = 0,                             # Tendency to take detours
                       oneWay = NULL,                           # ? Weird way of handling things down below
                       exit = exitLow,                          # Position of the exit
                       objects = objects,                       # Objects from which to take the number of objects
                       goalLines = goalLines,                   # Edges of the places where the goals are
                       goalRectangles = goalRectangles,         # ? Centers of the places where the goals are (I assume)
                       entry = entry,                           # Position where the pedestrian enters the space
                       pathPoints = pathPoints,                 # Waypoints through which the pedestrian can move
                       showTour = TRUE,                         # Whether to plot the path the pedestrian takes
                       tourDelay = 0 )                          # How much delay there should be in plotting it: Useful when running simulation, not when just plotting
...
iterations <- 200                                               # Maximal number of iterations to simulate
...
wme_directory <- function(x) {
    path <- file.path(".", "MinimalWorkingExample", x, "Trace1")
    if(!dir.exists(path)) {
...
for(i in seq_along(trace)) {
    ...
    img_list[[i]] <- image_read_pdf(filename)
...
image_write(image = img_animated,
            path = file.path(plot_directory, "trace1.gif"))
