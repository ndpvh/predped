...
few_edges <- predped::create_edges(c(0.3, 0.5), c(0, 13), setting, many_nodes = FALSE)
many_edges <- predped::create_edges(c(0.3, 0.5), c(0, 13), setting, many_nodes = TRUE)
...
```{r goals change, echo = FALSE, results = "asis"}

# Test
my_goal <- predped::goal(id = "goal exit", position = c(0, 13), counter = 10)

tbl <- (\() predped:::change(my_goal, setting, counter = 5)) |>
    test_times() |>
    rnd() |>
    matrix(nrow = 1)
...
caption <- paste0("Benchmark for the change method for goals.")
tbl |>
    knitr::kable(format = "latex", 
                 align = "lll",
                 caption = caption,
...
# Test 
tbl <- (\() predped:::goal_stack(10, setting, counter = 10)) |>
    test_times() |>
    rnd() |>
    matrix(nrow = 1)
colnames(tbl) <- c("2.5\\%", "50\\%", "95\\%")
