...
```{r objects enlarge, echo = FALSE, results = "asis"}
...
r <- (\() predped:::enlarge(obj[[1]], 0.5)) |>
    test_times() |>
    rnd()

c <- (\() predped:::enlarge(obj[[2]], 0.5)) |>
    test_times() |>
    rnd()

p <- (\() predped:::enlarge(obj[[3]], 0.5)) |>
    test_times() |>
    rnd()
...
# Test
r_r <- (\() predped::in_object(obj[[1]], pts, cpp = FALSE)) |>
    test_times() |>
    rnd() 
c_r <- (\() predped::in_object(obj[[2]], pts, cpp = FALSE)) |>
    test_times() |>
    rnd() 
p_r <- (\() predped::in_object(obj[[3]], pts, cpp = FALSE)) |>
    test_times() |>
    rnd() 


r_rcpp <- (\() predped::in_object(obj[[1]], pts, cpp = TRUE)) |>
    test_times() |>
    rnd() 
c_rcpp <- (\() predped::in_object(obj[[2]], pts, cpp = TRUE)) |>
    test_times() |>
    rnd() 
p_rcpp <- (\() predped::in_object(obj[[3]], pts, cpp = TRUE)) |>
    test_times() |>
    rnd() 
...
tbl <- c("cpp = FALSE", 
         "cpp = TRUE", 
         r_r[2], r_rcpp[2], 
         c(paste0("[", r_r[1], ",", r_r[3], "]")),  c(paste0("[", r_rcpp[1], ",", r_rcpp[3], "]")), 
         p_r[2], p_rcpp[2], 
         c(paste0("[", p_r[1], ",", p_r[3], "]")), c(paste0("[", p_rcpp[1], ",", p_rcpp[3], "]")), 
         c_r[2], c_rcpp[2], 
         c(paste0("[", c_r[1], ",", c_r[3], "]")), c(paste0("[", c_rcpp[1], ",", c_rcpp[3], "]"))) |>
    matrix(nrow = 2)
colnames(tbl) <- NULL
...
caption <- paste0("Benchmark for the in\\_object method for objects. ",
                  "Distinctions are made between the different objects that are exported by predped. ",
                  "We additionally distinguish between using the R version of the function (`cpp = FALSE`) ",
                  "or the Rcpp version (`cpp = TRUE`). ",
...
# Test
r_r <- (\() predped:::nodes_on_circumference(obj[[1]], cpp = FALSE)) |>
    test_times() |>
    rnd() 
c_r <- (\() predped:::nodes_on_circumference(obj[[2]], cpp = FALSE)) |>
    test_times() |>
    rnd() 
p_r <- (\() predped:::nodes_on_circumference(obj[[3]], cpp = FALSE)) |>
    test_times() |>
    rnd() 

r_rcpp <- (\() predped:::nodes_on_circumference(obj[[1]], cpp = TRUE)) |>
    test_times() |>
    rnd() 
c_rcpp <- (\() predped:::nodes_on_circumference(obj[[2]], cpp = TRUE)) |>
    test_times() |>
    rnd() 
p_rcpp <- (\() predped:::nodes_on_circumference(obj[[3]], cpp = TRUE)) |>
    test_times() |>
    rnd() 
...
tbl <- rbind(c("cpp = FALSE", 
             ...
             c("cpp = TRUE", 
               r_rcpp[2], 
               paste0("[", r_rcpp[1], ",", r_rcpp[3], "]"), 
               p_rcpp[2], 
               paste0("[", p_rcpp[1], ",", p_rcpp[3], "]"), 
               c_rcpp[2], 
               paste0("[", c_rcpp[1], ",", c_rcpp[3], "]")))
colnames(tbl) <- NULL

# Write the caption
caption <- paste0("Benchmark for the nodes\\_on\\_circumference method for objects. ",
                  "Distinctions are made between the different objects that are exported by predped. ",
                  "Additionally, we make the distinction between the Rcpp version ",
                  "of the function (cpp = TRUE) and the R version of the function ",
                  "(cpp = FALSE).")
tbl |>
    knitr::kable(format = "latex", 
                 align = "lll",
...
```{r objects out_object, echo = FALSE, results = "asis"}
...
# Test
r <- (\() predped::out_object(obj[[1]], pts)) |>
    test_times() |>
    rnd() 
c <- (\() predped::out_object(obj[[2]], pts)) |>
    test_times() |>
    rnd() 
p <- (\() predped::out_object(obj[[3]], pts)) |>
    test_times() |>
    rnd() 
...
# Write the caption
caption <- paste0("Benchmark for the out\\_object method for objects. ",
                  "Distinctions are made between the different objects that are exported by predped. ",
                  "For this benchmark, 100 points are evaluated.")
tbl |>
    knitr::kable(format = "latex", 
