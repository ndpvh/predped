...
vignette, we will dive deeper into tweaks one can do to further refine
simulations.
...
with $`X`$ number of agents in the space or continuing a previous
...
works if both the `state` and the model – here, variables
...
many at once (though this is not recommended). The argument
...
rather than in the middle of them to prevent agents moving to closed
gates before realizing they are not accessible.
...
you can pre-specify the behavior of the agents, meaning you do not
...
particular time. To achieve this, we have to specify the `fx` argument,
providing it with a function that takes in the state at each iteration
of the simulation and allows the user to make a set of desired changes
to this state. For our evacuation procedure, we create the function
`start_evacuation`, starting an evacuation after `100` iterations:
...
While we provide a lot of pre-specified agent types to choose from, one
may wish to play around with the parameters themselves to personalize
their own simulations. In this case, you can save the parameters for
future use by letting the
[`predped`](https://ndpvh.github.io/predped/reference/predped.html)
